...
zcat -f /var/log/auth.log* | grep -Eo $IP_REGEX | sort -u >> $TEMP_FILE

zcat -f /var/log/dovecot.log* | grep -Eo $IP_REGEX | sort -u >> $TEMP_FILE

zcat -f /var/log/exim4/mainlog* | grep -Eo $IP_REGEX | sort -u >> $TEMP_FILE
